...
- macOS 기본 셸인 zsh에서도 그대로 실행되는 명령을 사용한다. zsh의 읽기 전용 특수
  변수인 `status`에는 값을 대입하지 않는다. HTTP 상태를 변수에 담아야 한다면
  `http_status_code`, curl 종료 코드는 `curl_exit_code`처럼 의미가 분명하고 충돌하지
  않는 이름을 사용한다.
