...
- macOS 기본 셸인 zsh에서도 그대로 실행되는 명령을 사용한다. zsh에서 특별한 의미가
  있는 `path`·`status`·`pipestatus`·`fpath`·`cdpath` 같은 이름을 반복문 변수나 임시
  변수로 사용하지 않는다. 특히 `path`는 실행 파일 검색 경로인 `PATH`와 연결되어 있어
  덮어쓰면 같은 명령 안에서 `curl`·`sed` 등을 찾지 못할 수 있다.
- 임시 변수가 꼭 필요하면 다른 이름과 충돌하기 어려운 `geo_resource_name`,
  `geo_http_status_code`, `geo_curl_exit_code`처럼 `geo_` 접두어와 구체적인 뜻을 함께
  사용한다.
- 가능하면 아래처럼 각 `curl` 명령의 결과를 직접 확인하고, 여러 검사를 긴 한 줄의
  셸 명령이나 불필요한 변수 대입으로 묶지 않는다.
- 셸 문법이나 변수명 충돌처럼 외부 상태를 바꾸지 않은 실행 오류는 안전하게 바로잡아
  다시 확인한다. 재시도로 정상 확인됐다면 그 내부 오류를 진단 결과에 포함하거나
  사용자에게 별도 조치를 요구하지 않는다. 끝내 확인하지 못한 항목만 `확인 불가`로
  보고하고 이유를 짧게 설명한다.
